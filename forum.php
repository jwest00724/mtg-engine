...
switch($_GET['action']) {
	case 'board':
		boardView($db, $my, $mtg, $users, $board = 0);
		break;
	case 'topic':
		switch($_GET['sub']) {
			default:
				topicView($db, $my, $mtg, $users, $topic = 0);
				break;
			case 'new':
				topicNew($db, $my, $mtg, $users);
				break;
		}
		break;
	case 'post':
		respond($db, $my, $mtg, $users);
		break;
	case 'admin':
		switch($_GET['sub']) {
			case 'pin':
				...
				topicPin($db, $my, $mtg, $users, $logs);
				break;
			case 'lock':
				if(!$users->hasAccess('forum_topic_lock'))
					$mtg->error('You don\'t have access');
				topicLock($db, $my, $mtg, $users, $logs);
				break;
			case 'deletetopic':
				if(!$users->hasAccess('forum_topic_delete'))
					$mtg->error('You don\'t have access');
				topicDelete($db, $my, $mtg, $users, $logs);
				break;
			case 'editpost':
				postEdit($db, $my, $mtg, $users, $logs);
				break;
			case 'deletepost':
				postDelete($db, $my, $mtg, $users, $logs);
				break;
			default:
...
function boardView($db, $my, $mtg, $users, $board = 0) {
	global $pages;
	if($board)
		$_GET['ID'] = $board;
	if(empty($_GET['ID']))
...
function topicNew($db, $my, $mtg, $users) {
	if(empty($_GET['ID']))
		$mtg->error('You didn\'t specify a valid board');
...
function topicView($db, $my, $mtg, $users, $topic = 0) {
	global $pages, $parser;
	if($topic)
		$_GET['ID'] = $topic;
	if(empty($_GET['ID']))
	...
	$db->execute([$_GET['ID']]);
	$posts = $db->fetch_row();
	?><h3 class="content-subhead"><a href="forum.php?action=board&amp;ID=<?php echo $topic['parent_board'];?>"><?php echo $mtg->format($board['name']);?></a> &rarr; <?php echo $mtg->format($topic['name']);?></a></h3>
	<div class="small">
	...
	<table width="100%" class="pure-table pure-table-striped">
		...
		if(!count($posts))
			...
			foreach($posts as $post) {
				...
				?><tr><?php
					if($post['user']) {
						?><td><a id="<?php echo $post['id'];?>" style="display:none;"><?php echo $users->name($post['user']);?></a></td>
						<td><?php
...
function respond($db, $my, $mtg, $users) {
	...
	topicView($db, $my, $mtg, $users, $_GET['ID']);
}
function topicPin($db, $my, $mtg, $users, $logs) {
	if(empty($_GET['ID']))
		$mtg->error('You didn\'t specify a valid topic');
...
function topicLock($db, $my, $mtg, $users, $logs) {
	if(empty($_GET['ID']))
		$mtg->error('You didn\'t specify a valid topic');
...
function topicDelete($db, $my, $mtg, $users, $logs, $id = 0) {
	if($id)
		$_GET['id'] = $id;
	if(empty($_GET['ID']))
		$mtg->error('You didn\'t specify a valid topic');
	...
	boardView($db, $my, $mtg, $users, $row['parent_board']);
}
function postEdit($db, $my, $mtg, $users, $logs) {
	if(empty($_GET['ID']))
...
function postDelete($db, $my, $mtg, $users, $logs) {
	if(empty($_GET['ID']))
		$mtg->error('You didn\'t specify a valid post');
