...
if(!empty($_GET['heal'])) {
	...
	$db->query('UPDATE `users_stats` SET `health` = `health_max` WHERE id = ?');
	$db->execute([$_GET['heal']]);
	$db->query('UPDATE `users` SET `hospital` = 0 WHERE `id` = ?');
	$db->execute([$_GET['heal']]);
	$users->send_event($_GET['heal'], 'medical', '{id} healed you', $my['id']);
	$db->endTrans();
	$mtg->success('You\'ve healed '.$target.' for '.$mtg->format($cost).' point'.$mtg->s($cost));
}
...
<table width="100%" class="pure-table pure-table-striped">
	...
	if(!$db->num_rows())
		echo '<tr><td colspan="3" class="center">There are no patients</td></tr>';
	else {
		$rows = $db->fetch_row();
		foreach($rows as $row) {
			?><tr>
