...
class CodeDefinition {
	protected $tagName; /* NOTE: THIS PROPERTY SHOULD ALWAYS BE LOWERCASE; USE setTagName() TO ENSURE THIS */
	protected $useOption; /* Whether or not this CodeDefinition uses an option parameter. */
	protected $replacementText; /* The replacement text to be used for simple CodeDefinitions */
	protected $parseContent; /* Whether or not to parse elements of this definition's contents */
	protected $nestLimit; /* How many of this element type may be nested within each other */
	protected $elCounter; /* How many of this element type have been seen */
	protected $optionValidator; /* The input validator to run options through */
	protected $bodyValidator; /* The input validator to run the body ({param}) through */
	/**
	...
	public function __construct() {
		/* WARNING: This function is deprecated and will be made protected in a future version of jBBCode. */
	...
	public function hasValidInputs(ElementNode $el) {
		...
		if(!$this->parseContent() && $this->bodyValidator) { /* We only evaluate the content if we're not parsing the content. */
			$content = '';
			foreach($el->getChildren() as $child)
				$content .= $child->getAsBBCode();
			if(!$this->bodyValidator->validate($content))
				return false; /* The content of the element is not valid. */
		}
		return true;
	}
	/**
