...
class ElementNode extends Node {
	protected $tagName; /* The tagname of this element, for i.e. "b" in [b]bold[/b] */
	protected $attribute; /* The attribute, if any, of this element node */
	protected $children; /* The child nodes contained within this element */
	protected $codeDefinition; /* The code definition that defines this element's behavior */
	protected $nestDepth; /* How deeply this node is nested */
	/**
	...
	public function getAsBBCode() {
		...
		if(!empty($this->attribute))
			foreach($this->attribute as $key => $value)
				$str .= $key == $this->tagName ? '=' . $value : ' ' . $key . '=' . $value;
		$str .= ']';
		foreach($this->getChildren() as $child)
	...
	public function closestParentOfType($str) {
		...
		return strtolower($currentEl->getTagName()) != $str ? null : $currentEl;
	}
}