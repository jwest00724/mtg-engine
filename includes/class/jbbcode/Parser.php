...
namespace JBBCode;
$files = array('ElementNode', 'TextNode', 'DefaultCodeDefinitionSet', 'DocumentElement', 'CodeDefinition', 'CodeDefinitionBuilder', 'CodeDefinitionSet', 'NodeVisitor', 'ParserException', 'Tokenizer', 'visitors/NestLimitVisitor', 'InputValidator');
...
class Parser {
	...
	protected $treeRoot; /* The root element of the parse tree */
	protected $bbcodes; /* The list of bbcodes to be used by the parser. */
	protected $nextNodeid; /* The next node id to use. This is used while parsing. */
	...
	public function parse($str) {
		$this->reset(); /* Set the tree root back to a fresh DocumentElement. */
		...
		while($tokenizer->hasNext()) {
			...
			if($parent->getCodeDefinition() && false === $parent->getCodeDefinition()->parseContent()) {
				$this->parseAsTextUntilClose($parent, $tokenizer); /* We're inside an element that does not allow its contents to be parseable. */
	...
	protected function reset() {
		$this->treeRoot = new DocumentElement(); // remove any old tree information
		$this->nextNodeid = 1; /* The document element is created with nodeid 0. */
	}
	...
	protected function parseStartState(ElementNode $parent, Tokenizer $tokenizer) {
		...
		if('[' == $next)
			...
			$this->createTextNode($parent, $next); /* Drop back into the main parse loop which will call this same method again. */
			return $parent;
		}
	}
	...
	protected function parseTagOpen(ElementNode $parent, Tokenizer $tokenizer) {
		if(!$tokenizer->hasNext()) {
			$this->createTextNode($parent, '['); /* The [ that sent us to this state was just a trailing [, not the opening for a new tag. Treat it as such. */
			return $parent;
		}
		 ...
		 */
		while('[' == $next) {
			$this->createTextNode($parent, '['); /* The previous [ was just a random bracket that should be treated as text. Continue until we get a non open bracket. */
			if(!$tokenizer->hasNext()) {
		...
		if(']' == $next) {
			...
			return $this->parseTag($parent, $tokenizer, $next); /* $next is plain text... likely a tag name. */
	}
	...
	protected function parseTag(ElementNode $parent, Tokenizer $tokenizer, $tagContent) {
		...
		if(!$tokenizer->hasNext() || ($next = $tokenizer->next()) != ']') {
			$this->createTextNode($parent, '['); /* This is a malformed tag. Both the previous [ and the tagContent is really just plain text. */
		...
		$actualTagName;
		$actualTagName = ('' != $tmpTagName && '/' == $tmpTagName[0])  ? substr($tmpTagName, 1) : $tmpTagName; /* This is a closing tag name. */
		...
		if(!empty($options))
			$el->setAttribute($options); /* We have an attribute we should save. */
		$parent->addChild($el);
		return $el;
	}
